---
title: "Introduction to the nnest"
...
This vignette is going to first look at the structure and assembly of a `nnest`, then examine how we can work with the nnest and utilize built in functions to navigate the environment.

A `nnest` is what is generated during an experiment. Each potential data point you wanting to add to your dataset gets its own `nnest`, and each `nnest` is built throughout an experiment. The `nnest` is comprised of everything relevant to the specific piece of data in a dataset, assembled into a well organized nested list that is easy to access and apply analytical funtions to. 
...
The first `nnest` we will look at is the built in `field` dataset from experiments performed in the dentate gyrus of the rat hippocampus to asses a type of synaptic plasticity called long-term depression (LTD). The working directory (`field$wd`) gives us an idea of how this project was structured.
...
Built in `nphys::proj_fun` functions can be used to update data paths, drawing on the UI spreadsheet params. The function `nphys::proj_dir()` will assemble an data directory based upon input from the params spreadsheet, into `dir/Protocol/ExperimentID/Group/Treatment/Date_ID1` 
...
Loading the dataset imports a nested list into the REnvironment that contains all of the information and variables needed to quickly analyze our data and keep track of it in a reliable way. 

The `field` dataset is an example of what shape a data will look like after you have collected your raw data, added it to your project folderadding it to your project.
